"""Shared helpers for staged skims preparation and grid allocation inputs."""
...
def _build_source_activity_totals(skims_df: pd.DataFrame) -> Optional[pd.DataFrame]:
    ...
    activity["countyfp"] = _normalize_county_fips(activity["countyfp"])
...
def resolve_prepared_skims_path(input_root: Path) -> Optional[str]:
    ...
    for name in ["prepared_skims_for_grid_allocation.parquet", "prepared_skims_for_grid_allocation.csv.gz"]:
        candidate = skims_dir / name
        if candidate.exists():
            return str(candidate)
    return None


def _resolve_staged_skims_input_path(input_root: Path) -> Optional[str]:
    skims_dir = input_root / "skims"
    preferred = [
    ...
    for pattern in patterns:
        matches = sorted(path for path in skims_dir.rglob(pattern) if path.is_file())
        if matches:
            return str(matches[0])
    return None
    ...
    *,
    ...
    if not skims_input_source:
        if not intersection_path:
            raise ValueError(
                "intersection_path is required for the maintained events fallback path."
            )
        if not simulation_network_folder:
            raise FileNotFoundError(
                "No skims file found and simulation_network_folder not provided for events fallback."
            )
        logger.info("Step 1: no staged skims found — building staged skims from local events")
        ...
        if event_inputs is None:
            raise FileNotFoundError(
                "No skimsEmissions or events file found under configured simulation_network_folder."
            )
