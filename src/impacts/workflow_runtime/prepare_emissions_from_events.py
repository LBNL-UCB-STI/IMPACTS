"""Helpers for deriving staged skims inputs from BEAM events.
...
aermod_srv_cell_id : int — AERMOD grid cell id
inmap_srm_cell_id  : int — InMAP/ISRM grid cell id
...
def _normalize_rate_dimensions(rates: pd.DataFrame) -> pd.DataFrame:
    ...
    if "countyfp" in result.columns:
        result["countyfp"] = result["countyfp"].astype("string").str.extract(r"(\d+)")[0].astype("string")
        result["countyfp"] = result["countyfp"].where(result["countyfp"].isna(), result["countyfp"].str.zfill(3))
...
def _enrich_with_network(skims: pd.DataFrame, network_path: str) -> pd.DataFrame:
    ...
    network = pd.read_csv(network_path, usecols=["linkId", "attributeOrigType"])
...
def _enrich_with_intersection(skims: pd.DataFrame, intersection_path: str) -> pd.DataFrame:
    ...
    """
    available = pd.read_parquet(intersection_path, columns=_INTERSECTION_ZONE_COLS)
...
def find_staged_events_path(input_root: Path) -> Optional[str]:
    patterns = [
...
def build_staged_skims_from_events(
    *,
    ...
    events_path = find_staged_events_path(input_root)
    if not events_path:
        return None
    ...
    logger.info("Step 1: wrote staged skims from events %d rows → %s", len(skims_df), skims_path)
    return str(skims_path)


def build_staged_skims_from_local_events(
    *,
    input_root: Path,
    ...
    events_path = resolve_latest_events_local_path(simulation_network_folder)
    if not events_path:
        return None

    logger.info("Step 1: building skims from local events %s", events_path)
    skims_df = build_skims_from_events(
        events_path,
        network_path=network_path,
        intersection_path=intersection_path,
    ...
    skims_path = _write_staged_skims(skims_df, input_root=input_root)
    logger.info("Step 1: wrote staged skims from local events %d rows → %s", len(skims_df), skims_path)
    return events_path, str(skims_path), skims_df
...
def prepare_events_inputs(
    *,
    ...
    """Build and stage prepared skims/activity tables from local BEAM events."""
    ...
    if staged is None:
        logger.info("Step 1: no events file found under %s", simulation_network_folder)
        return None
