"""Step 3 — InMAP concentration workflow."""
...
def _trace_columns(step: str, label: str, columns: list[str]) -> None:
    ...
    logger.info("%s trace %s columns(%d): %s%s", _step_label(3, step), label, len(columns), preview, suffix)


def _trace_frame(step: str, label: str, df: pd.DataFrame, *, key_cols: Optional[list[str]] = None) -> None:
    logger.info("%s trace %s shape=%s", _step_label(3, step), label, df.shape)
    ...
    if key_cols:
        ...
        if present and not df.empty:
            ...
            logger.info("%s trace %s sample_keys=%s", _step_label(3, step), label, sample)
...
def _trace_array(step: str, label: str, values: np.ndarray) -> None:
    ...
    if size == 0:
        logger.info("%s trace %s length=0", _step_label(3, step), label)
        return
    logger.info(
...
def _prepare_grid_emissions(
    ...
    logger.info("%s trace source_column_map=%s", _step_label(3, "0"), source_column_map)
    _trace_frame("0", "prepared_grid_emissions", grouped, key_cols=["GRID"])
    logger.info("%s trace available_source_pollutants=%s", _step_label(3, "0"), sorted(available_pollutants))
...
def _align_emissions_to_isrm_sources(
    ...
    """Step 3.1: align emissions source cells to the ISRM source index."""
...
def _compute_no2_response(
    *,
    ...
    logger.info("%s resolving NO2 source from ISRM zarr or configured fallback matrix", _step_label(3, "1"))
    if "NO2" in sr:
        logger.info("%s using NO2 transfer matrix from ISRM zarr", _step_label(3, "1"))
    ...
    logger.info("%s using configured ISRM NOx->NO2 fallback transfer matrix", _step_label(3, "1"))
...
def _read_receptor_vector(sr, key: str, receptor_dim: int) -> np.ndarray:
    ...
    if values.shape[0] < receptor_dim:
        raise ValueError(
            f"{_step_label(3, '3')} {key} length {values.shape[0]} is shorter than receptor_dim {receptor_dim}"
        )
...
def _assemble_concentration_results(
    *,
    ...
    """Step 3.3: assemble concentration outputs on the receptor dimension."""
    receptor_dim = int(sr["SOA"].shape[2])
    logger.info("%s assembling concentration DataFrame for %d receptors", _step_label(3, "3"), receptor_dim)
    for name, arr in arrays.items():
        logger.info("%s %s response length=%d", _step_label(3, "3"), name, int(np.asarray(arr).shape[0]))
    ...
    if bad:
        raise ValueError(
            f"{_step_label(3, '3')} produced mismatched receptor lengths: expected {receptor_dim}, got {bad}"
        )
...
def _build_beam_inmap_concentrations_gdf(
    *,
    ...
    if grid_id_col not in grid.columns:
        raise ValueError(
            f"{_step_label(3, '4')} grid id column '{grid_id_col}' not found in {inmap_grid_path}. "
...
def _write_concentration_outputs(
    ...
    if out.suffix.lower() != ".parquet":
        raise ValueError("Output path must end with .parquet for Step 3 BEAM InMAP concentrations")
    gpkg_path = out.with_suffix(".gpkg")
    logger.info("%s writing BEAM InMAP concentrations GeoParquet to %s", _step_label(3, "5"), out)
    beam_inmap_concentrations_gdf.to_parquet(out, index=False)
    logger.info("%s writing BEAM InMAP concentrations GPKG to %s", _step_label(3, "5"), gpkg_path)
    ...
    *,
    ...
    with logging_redirect_tqdm():
        for concentration_name, spec in tqdm(
            concentration_plan,
            desc="Step 3.2 pollutant responses",
            unit="species",
            file=sys.stdout,
            dynamic_ncols=True,
            leave=True,
...
def run(
    *,
    ...
    """Step 3: compute and export receptor-side InMAP concentrations from BEAM InMAP emissions."""
    ...
    logger.info("%s loading BEAM emissions for InMAP from %s", _step_label(3, "0"), emissions_input_path)
    ...
    logger.info("%s loading ISRM store from %s", _step_label(3, "0"), pipeline.isrm_url)
