...
output_file_with_isrm <- "K:/BREATHE/ISRM/MyProject-riv/IE_full_concentration_all_grids_dynamicPattern6x6_withISRM.csv"
...
# =========================================================
# 11-14. SOURCE × KERNEL in chunks, aggregate immediately
#        reduce RAM usage
# =========================================================

chunk_size <- 2000   # change it based on RAM

conc_total <- data.table(
  gid_internal = integer(),
  gridID = integer(),
  concentration_PM25 = numeric(),
  concentration_NOx  = numeric(),
  concentration_ROG  = numeric(),
  concentration_SOx  = numeric(),
  concentration_NH3  = numeric()
)

src_keys <- unique(src$pattern_key)

for (k in src_keys) {
  cat("Processing pattern:", k, "\n")
  
  src_sub_all <- src[pattern_key == k]
  ker_sub <- copy(kernel_list[[k]])
  
  if (nrow(src_sub_all) == 0 || nrow(ker_sub) == 0) next
  
  n_chunks <- ceiling(nrow(src_sub_all) / chunk_size)
  
  for (ii in seq_len(n_chunks)) {
    i1 <- (ii - 1) * chunk_size + 1
    i2 <- min(ii * chunk_size, nrow(src_sub_all))
    
    cat("  chunk", ii, "/", n_chunks, "rows", i1, "to", i2, "\n")
    
    src_sub <- src_sub_all[i1:i2]
    ...
    setkey(exp_sub, target_ix, target_iy)
    
    exp_sub <- target_index[
      exp_sub,
      on = .(ix = target_ix, iy = target_iy),
      nomatch = 0L,
      allow.cartesian = TRUE
    ...
    setnames(
      exp_sub,
      old = c("gid_internal", "gridID"),
      new = c("target_gid_internal", "target_gridID"),
      skip_absent = TRUE
    )
    
    # calculate conc contribution
    exp_sub[, `:=`(
      add_PM25 = emis_PM25 * resp,
      add_NOx  = emis_NOx  * resp,
      add_ROG  = emis_ROG  * resp,
    ...
    conc_sub <- exp_sub[, .(
      ...
      concentration_SOx  = sum(add_SOx,  na.rm = TRUE),
      concentration_NH3  = sum(add_NH3,  na.rm = TRUE)
    ), by = .(gid_internal = target_gid_internal, gridID = target_gridID)]
    
    # add to master table
    conc_total <- rbindlist(list(conc_total, conc_sub), use.names = TRUE, fill = TRUE)
    
    # clean ram 
    rm(src_sub, exp_sub, conc_sub)
    gc()
  }
}
...
# aggregate all chunk
conc <- conc_total[, .(
  concentration_PM25 = sum(concentration_PM25, na.rm = TRUE),
  concentration_NOx  = sum(concentration_NOx,  na.rm = TRUE),
  concentration_ROG  = sum(concentration_ROG,  na.rm = TRUE),
  concentration_SOx  = sum(concentration_SOx,  na.rm = TRUE),
  concentration_NH3  = sum(concentration_NH3,  na.rm = TRUE)
), by = .(gid_internal, gridID)]
...
ISRM_Product <- ISRM_Product[, .(
  gid, isrm,pNO3, pNH4, SOA, pSO4, PrimaryPM25, secondaryPM25, TotalPM25
)]
...
)]   ### InMAP Primary pm2.5 in non-aermod grid

result_out[, newTotalPM25 := PrimaryPM25_integrated_ISRM + secondaryPM25]
result_out[, newTotalPM25_no_InMAP_pPM25 := concentration_PM25 + secondaryPM25]
