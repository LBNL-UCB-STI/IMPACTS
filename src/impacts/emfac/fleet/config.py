...
def _ingest_configured_sources(config: dict) -> dict:
    ...
    if isinstance(emfac, dict):
        emfac["outputs"] = _normalize_configured_path(
            emfac.get("outputs"),
            path_label="emfac.outputs",
            must_exist=False,
        )
        emfac["rates_file"] = _normalize_configured_path(
            emfac.get("rates_file"),
            path_label="emfac.rates_file",
            must_exist=False,
        )
        emfac["activity_file"] = _normalize_configured_path(
            emfac.get("activity_file"),
            path_label="emfac.activity_file",
            must_exist=False,
        )
        emfac["fleet_file"] = _normalize_configured_path(
            emfac.get("fleet_file"),
            path_label="emfac.fleet_file",
            must_exist=False,
        )
        activities = emfac.get("activities")
        if isinstance(activities, dict):
            emfac["activities"] = deepcopy(activities)
