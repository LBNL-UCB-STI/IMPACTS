"""Fleet Step 4: map EMFAC passenger distributions onto BEAM passenger vehicles.

Substeps:
4.1 Parse and rebuild BEAM passenger probability strings.
4.2 Normalize BEAM passenger sampling distributions.
4.3 Optionally crosswalk EMFAC classes through ATLAS body types.
4.4 Map cars, bikes, and buses into EMFAC-backed vehicle types.
4.5 Regenerate the passenger vehicle file from mapped vehicle types.
"""
...
# Step 4.1: probability-string parsing and formatting
...
# Step 4.2: normalize BEAM passenger probability inputs
...
# Step 4.3: optional ATLAS crosswalk path for passenger cars
...
# Step 4.4-4.5: build mapped passenger vehicle types and passenger fleet
...
def run_step4(workflow: dict[str, Any]) -> dict[str, Any]:
    """Step 4: map passenger fleet records and regenerate the passenger vehicles file."""
    config = workflow["config"]
    work_dir = workflow["work_dir"]
    format_beam_vehicle_types = workflow.get("format_beam_vehicle_types") or _build_beam_vehicle_formatter(config)
