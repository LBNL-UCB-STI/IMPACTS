...
def _map_one(isrm_source_grid: str) -> Optional[pd.DataFrame]:
    return functions.map_to_isrm(_DATDIR, isrm_source_grid, _ISRM, _BOUNDING_BOX)
...
def main() -> None:
    ...
    results = []
    with Pool(processes=6, initializer=_init_worker, initargs=(isrm_path, bbox_wkb, datdir)) as pool:
