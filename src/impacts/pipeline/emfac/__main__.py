...
from impacts.pipeline.emfac.fleet.main import main as run_fleet


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m impacts.pipeline.emfac",
    ...
    parser.add_argument(
        ...
        "'python -m impacts.pipeline.emfac --config settings.yaml', "
        "'python -m impacts.pipeline.emfac activities --config settings.yaml', "
        "'python -m impacts.pipeline.emfac fleet --activities-manifest activities_manifest.yaml'.",
    )
    parser.add_argument(
        "--config",
        dest="config_path",
...
def main(argv: list[str] | None = None) -> None:
    ...
    if workflow == "activities":
        if not config_path:
            raise SystemExit("--config is required. Usage: python -m impacts.pipeline.emfac activities --config <settings.yaml>")
        _ensure_activities(config_path)
        return
    if workflow == "fleet":
        if not activities_manifest_path:
            raise SystemExit(
                "--activities-manifest is required. Usage: python -m impacts.pipeline.emfac fleet --activities-manifest <activities_manifest.yaml>"
            )
        run_fleet(activities_manifest_path=activities_manifest_path)
        return
    if not config_path:
        raise SystemExit("--config is required. Usage: python -m impacts.pipeline.emfac --config <settings.yaml>")
